...
| --- | --- | --- |
| **Use out-of-the-box** | Pre-built AI experiences across SAP and Microsoft apps | Joule (SAP), Microsoft 365 Copilot |
| **Extend with custom agents** | Build company-specific agents with low-code/pro-code | Copilot Studio, M365 Agents SDK |
...
These layers are not mutually exclusive — most organizations use a combination depending on the business process and complexity involved.
...
- **How it works:** Agent → MCP protocol (JSON-RPC 2.0) → MCP server (e.g., FastAPI backend) → SAP APIs (OData, RFC, BAPI)
...
- Integrate with M365 Copilot Employee Self-Service agent
...
A key pattern emerging from enterprise deployments is **multi-agent orchestration** — where M365 Copilot acts as the front-end and orchestrates multiple specialized agents:
